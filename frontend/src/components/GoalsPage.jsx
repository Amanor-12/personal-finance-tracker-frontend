...
function GoalsPage({ currentUser, onLogout }) {
  ...
  const [goalGuidance, setGoalGuidance] = useState(null);
  const [goalGuidanceError, setGoalGuidanceError] = useState('');
  const [isGoalGuidanceLoading, setIsGoalGuidanceLoading] = useState(false);
  ...
  const handleGenerateGoalGuidance = async () => {
    if (!hasProGoalGuidance) {
      navigate('/pricing');
      return;
    }

    setIsGoalGuidanceLoading(true);
    setGoalGuidanceError('');

    try {
      const nextGuidance = await aiStore.getGoalGuidance(nextGoalFocus?.id || null);
      setGoalGuidance(nextGuidance);
    } catch (error) {
      if (error.status === 401) {
        await onLogout();
        return;
      ...
      setGoalGuidance(null);
      setGoalGuidanceError(error.message || 'AI goal guidance could not load.');
    } finally {
      setIsGoalGuidanceLoading(false);
    }
  };
      ...
      >
        ...
        <section className="goals-portfolio-secondary" aria-label="Goal portfolio summary">
          {!isBillingLoading && goalLimit !== null ? (
            <ResourceLimitCard
              body="Free workspaces can track a focused set of targets. Move to Plus for unlimited goals, then Pro for denser milestone guidance."
              limit={goalLimit}
              resourceLabel="goals"
              usage={goalUsage}
            />
          ...
          <article className="finance-intelligence-card">
            <div className="finance-intelligence-head">
              <div>
                ...
                <h3>{hasProGoalGuidance ? 'Pro milestone guidance is live' : 'Pro adds milestone guidance'}</h3>
              </div>
              {!hasProGoalGuidance ? (
            ...
            <div className="finance-intelligence-list">
              {(hasProGoalGuidance ? milestoneSignals : milestoneSignals.slice(0, 2)).map((signal) => (
          ...
          <article className="finance-intelligence-card">
            <div className="finance-intelligence-head">
              ...
              <button
                ...
                type="button"
                onClick={handleGenerateGoalGuidance}
                disabled={hasProGoalGuidance ? isGoalGuidanceLoading || !goals.length : false}
              >
                {hasProGoalGuidance ? (isGoalGuidanceLoading ? 'Generating...' : 'Generate guidance') : 'Unlock guidance'}
              </button>
            </div>
            <p className="finance-intelligence-copy">
              {hasProGoalGuidance
            ...
            {!isGoalGuidanceLoading && goalGuidanceError ? (
              <div className="finance-intelligence-list">
                <article className="finance-intelligence-row tone-watch">
                  <strong>AI guidance unavailable</strong>
                  <p>{goalGuidanceError}</p>
                </article>
              </div>
            ) : null}
            {!isGoalGuidanceLoading && !goalGuidanceError && goalGuidance ? (
              <div className="finance-intelligence-list">
            ...
            {!isGoalGuidanceLoading && !goalGuidanceError && !goalGuidance ? (
              <div className="finance-intelligence-list">
                <article className="finance-intelligence-row tone-neutral">
